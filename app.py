...
# download_audio_yt_dlp remains unchanged
def download_audio_yt_dlp(url: str) -> tuple[str | None, str | None]:
    ...
    except Exception as e: st.error(f"Failed to create temporary file: {e}", icon="❌"); return None, None
    ydl_opts = {
    ...
    progress_placeholder = st.empty(); progress_placeholder.info("Download in progress...")
    def progress_hook(d):
        ...
        elif hook_status == 'error': progress_placeholder.error("Download hook reported an error.", icon="⚠️")
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                if not os.path.exists(actual_filepath) or os.path.getsize(actual_filepath) == 0:
                    st.error(f"Download failed: output file missing/empty.", icon="❌"); progress_placeholder.empty()
                    if os.path.exists(actual_filepath): os.remove(actual_filepath)
                    return None, None
                st.success(f"Audio download completed: '{video_title}' ({os.path.getsize(actual_filepath)/1024/1024:.2f} MB).")
                progress_placeholder.empty(); return actual_filepath, video_title
            except yt_dlp.utils.DownloadError as e:
                st.error(f"yt-dlp download failed: {e}. Check logs.", icon="❌"); progress_placeholder.empty();
...
async def transcribe_audio_data(audio_data: bytes, language_code: str, filename_hint: str = "audio") -> str:
    ...
    try:
        ...
        # --- Use 'base' model and add punctuate/numerals ---
        options: PrerecordedOptions = PrerecordedOptions(
            model="base", # <<< Use base model
            smart_format=True,
            language=language_code,
            punctuate=True, # <<< Add punctuation
            numerals=True,  # <<< Add numeral conversion
        )
        # --- End of modification ---
        st.info(f"Sending '{filename_hint}' (approx {len(audio_data)/1024:.1f} KB) to Deepgram ({language_code}, model: base)...", icon="📤")
        ...
        if transcript:
             ...
             # More specific warning based on model attempt
             st.warning("Transcription completed using 'base' model, but no transcript text found in the response.", icon="⚠️")
             return "[Transcription empty or failed]"
    except Exception as e:
        st.error(f"Deepgram transcription failed (using 'base' model):", icon="❌")
        st.exception(e)
...
def create_word_document(text: str) -> io.BytesIO | None:
    ...
    except Exception as e: st.error(f"Failed to create Word document: {e}", icon="📄"); return None

# sanitize_filename remains unchanged
def sanitize_filename(filename: str) -> str:
    """Removes potentially problematic characters for filenames."""
    if not filename: return "transcript"
    base_name=os.path.splitext(filename)[0]; sanitized=re.sub(r'[<>:"/\\|?*\s\.\t\n\r\f\v]+','_',base_name)
    sanitized=sanitized.strip('_-'); sanitized=sanitized if sanitized else "transcript"; return sanitized[:100]
...
with col1: youtube_url = st.text_input("Enter YouTube URL:", placeholder="e.g., https://www.youtube.com/watch?v=...")
with col2: selected_language_name = st.selectbox("Audio Language:", options=list(SUPPORTED_LANGUAGES.keys()), index=0, help="Select the primary language spoken in the video.")
...
if youtube_url:
    ...
    if transcribe_button and not st.session_state.processing:
        if not (youtube_url.startswith("http://") or youtube_url.startswith("https://")):
            ...
            st.session_state.processing = True; st.session_state.transcript = None
            st.session_state.show_result = False; st.session_state.current_url = youtube_url
            st.session_state.video_title = "transcript"; st.rerun()

if st.session_state.processing:
    url_to_process=st.session_state.current_url; lang_code_to_process=selected_language_code
    if not url_to_process:
        st.error("Internal state error: URL missing."); st.session_state.processing=False
        ...
        with st.spinner(f"Step 1/2: Downloading audio for '{url_to_process[:50]}...'"):
            try:
                ...
                st.error(f"Error during audio download phase: {e}", icon="❌"); st.session_state.processing = False
                st.session_state.show_result = True; st.session_state.transcript = "[Download Error]"; st.rerun()
        if audio_filepath and os.path.exists(audio_filepath):
            with st.spinner(f"Step 2/2: Transcribing audio using Deepgram ({lang_code_to_process}, model: base)..."):
                try:
                    st.info("Reading downloaded audio data...", icon="🎧")
                    with open(audio_filepath, "rb") as audio_file: audio_data = audio_file.read()
                    if not audio_data:
                        st.error("Failed to read audio data.", icon="⚠️"); transcript_text = "[File Read Error]"
                    else:
                    ...
                    st.error(f"Error during transcription phase: {e}", icon="❌"); transcript_text = "[Transcription Error]"
                finally:
            ...
            if audio_filepath is None : st.warning("Transcription skipped: download failed/interrupted.", icon="⚠️")
            else: st.warning("Transcription skipped: download failed (file missing/empty).", icon="⚠️")
...
if st.session_state.show_result:
    ...
    if st.session_state.transcript and st.session_state.transcript not in [
        ...
        if not result_message: result_message = "Transcription result is empty. No speech detected or wrong language selected?"
        elif result_message == "[Transcription empty or failed]": result_message = "Transcription result is empty or failed (tried 'base' model). No speech detected or wrong language selected?"
        elif result_message == "[Download Error]" or result_message == "[Download Failed]": result_message = "Transcript generation failed: Audio download error."
        elif result_message == "[File Read Error]": result_message = "Transcript generation failed: Cannot read downloaded audio file."
