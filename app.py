...
st.write("DEBUG: Checkpoint 1 - After warning") # <<< DEBUG ADDED

# --- Configuration ---
@st.cache_data
def load_api_key():
    st.write("DEBUG: Entering load_api_key function") # <<< DEBUG ADDED
    try:
        api_key = st.secrets["DEEPGRAM_API_KEY"];
        if not api_key or api_key == "YOUR_DEEPGRAM_API_KEY_HERE" or len(api_key) < 20:
            st.error("Error: DEEPGRAM_API_KEY missing/invalid in secrets.", icon="🚨"); return None
        st.write("DEBUG: API Key seems present in secrets") # <<< DEBUG ADDED
        return api_key
    except KeyError: st.error("Error: DEEPGRAM_API_KEY key not found in secrets.", icon="🚨"); return None
    except Exception as e: st.error(f"Secrets error: {e}", icon="🚨"); return None

DEEPGRAM_API_KEY = load_api_key()

st.write(f"DEBUG: Checkpoint 2 - After load_api_key call. Key is None? {DEEPGRAM_API_KEY is None}") # <<< DEBUG ADDED

if not DEEPGRAM_API_KEY:
    st.write("DEBUG: API Key is None, stopping execution.") # <<< DEBUG ADDED
    st.stop() # Keep stop

st.write("DEBUG: Checkpoint 3 - API Key seems valid, proceeding.") # <<< DEBUG ADDED

@st.cache_resource
def get_deepgram_client(api_key):
    st.write("DEBUG: Entering get_deepgram_client function") # <<< DEBUG ADDED
    try:
        config=DeepgramClientOptions(verbose=False)
        deepgram=DeepgramClient(api_key,config)
        st.write("DEBUG: Deepgram client initialized successfully.") # <<< DEBUG ADDED
        return deepgram
    except Exception as e:
        st.error(f"Deepgram client init error: {e}", icon="🚨")
        st.exception(e) # Show traceback for init error
        st.stop() # Stop if client fails

deepgram = get_deepgram_client(DEEPGRAM_API_KEY)

st.write("DEBUG: Checkpoint 4 - After get_deepgram_client call.") # <<< DEBUG ADDED
...
def download_audio_yt_dlp(url: str) -> tuple[str | None, str | None]:
    ...
    def progress_hook(d):
        ...
        elif hook_status == 'finished': progress_placeholder.info("Download finished. Converting to WAV...");
            ...
            try:
                ...
                if os.path.exists(potential_final_path) and os.path.getsize(potential_final_path) > 0:
                    ...
                    return final_audio_path, video_title
                     ...
                     final_audio_path = temp_audio_intermediate_path; st.warning("WAV conversion output questionable, using original path.", icon="⚠️"); progress_placeholder.empty()
                     return final_audio_path, video_title
                else:
...
st.write("DEBUG: Checkpoint 5 - After helper function definitions.") # <<< DEBUG ADDED

# --- Streamlit App UI ---
st.write("DEBUG: Checkpoint 6 - Defining UI elements.") # <<< DEBUG ADDED
...
st.write("DEBUG: Checkpoint 7 - After UI inputs defined.") # <<< DEBUG ADDED
...
st.write("DEBUG: Checkpoint 8 - End of script.") # <<< DEBUG ADDED
