...
def load_api_key():
    ...
    try: # <<< try starts here
        ...
        if not api_key or api_key == "YOUR_DEEPGRAM_API_KEY_HERE" or len(api_key) < 20:
            ...
            return None
        # This return is part of the 'try' block
        return api_key
    # --- Corrected Indentation for except blocks ---
    except KeyError: # <<< Correctly aligned with 'try'
...
# Footer
st.markdown("---")
# Adding date/time info as requested
current_time_utc = datetime.utcnow()
current_time_local = datetime.now() # Server's local time
st.caption(f"Powered by Deepgram, yt-dlp, Streamlit | UTC: {current_time_utc.strftime('%Y-%m-%d %H:%M:%S')} | Server Time: {current_time_local.strftime('%Y-%m-%d %H:%M:%S %Z')}")
